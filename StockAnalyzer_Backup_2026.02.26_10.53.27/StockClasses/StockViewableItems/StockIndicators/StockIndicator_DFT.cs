...
{
    public class StockIndicator_DFT : StockIndicatorBase
    {
        public StockIndicator_DFT()
        {
        }
        ...
        {
            get { return "DFT(" + this.Parameters[0].ToString() + ")"; }
        }

        public override string Definition
        {
            get { return "DFT(int Period)"; }
        }
        ...
        {
            get { return new ParamRange[] { new ParamRangeInt(1, 5000) }; }
        }
        public override string[] ParameterNames
        {
            get { return new string[] { "Period" }; }
        }
        public override string[] SerieNames { get { return new string[] { "DFT(" + this.Parameters[0].ToString() + ")" }; } }
        ...
        public override void ApplyTo(StockSerie stockSerie)
        {
            ...
            Complex[] input = new Complex[stockSerie.Count];
            for (int i = 0; i < stockSerie.Count; i++)
            {
                input[i] = new Complex(closeSerie[i], 0);
            }

            FourierTransform.DFT(input, FourierTransform.Direction.Forward);

            int halfCount = stockSerie.Count / 2;

            double step = Math.PI / halfCount / 2.0;

            // Smooth
            for (int i = 0; i < period; i++)
            {
                double factor = Math.Cos(i * step);

                input[halfCount + i].Im = 0;
                input[halfCount + i].Re = 0;
                input[halfCount - i].Im = 0;
                input[halfCount - i].Re = 0;
            }
            ...
            //    input[i].Im *= factor;
            //    input[i].Re *= factor;
            //    input[stockSerie.Count - i - 1].Im *= factor;
            //    input[stockSerie.Count - i - 1].Re *= factor;
            //}
            input[halfCount].Im = 0;
            input[halfCount].Re = 0;

            FourierTransform.DFT(input, FourierTransform.Direction.Backward);

            FloatSerie maSerie = new FloatSerie(input.Select(c => (float)c.Re).ToArray(), "DFT");
