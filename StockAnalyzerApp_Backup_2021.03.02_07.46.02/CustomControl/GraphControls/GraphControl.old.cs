...
namespace StockAnalyzerApp.CustomControl.GraphControls
{
    ...
    public partial class GraphControl : Panel
    {
        ...
        public GraphDrawMode DrawingMode { get; set; }
        ...
        protected void DrawSelectionZone(System.Windows.Forms.MouseEventArgs e)
        {
            using (MethodLogger ml = new MethodLogger(this))
            {
                ...
                if (e.Location.Y < mouseDownPos.Y)
                {
                    ...
                    this.DrawString(foregroundGraphic, "+" + variation.ToString("P2"), axisFont, Brushes.Green, this.backgroundBrush, new PointF(x + width + 4, y), true);
                    ...
                    if (height >= 30)
                    {
                        ...
                        {
                            fiboY = y + height * fibo;
                            this.DrawString(foregroundGraphic, fibo.ToString("P2"), axisFont, Brushes.Green, this.backgroundBrush, x - 36, fiboY - 5, false);
                            if (fibo == 0.5f)
                            {
                                greenPen.Width = 2;
                            }
                ...
                else
                {
                    ...
                    this.DrawString(this.foregroundGraphic, variation.ToString("P2"), axisFont, Brushes.Red, this.backgroundBrush, new PointF(x + width + 4, y + height), true);
        ...
        protected void DrawTmpItem(Graphics graph, Pen pen, DrawingItem item, bool useTransform, bool temporary)
        {
            ...
            if (useTransform)
            {
                item.Draw(graph, pen, this.matrixValueToScreen, rect2D);
            }
            else
            {
                item.Draw(graph, pen, GraphControl.matrixIdentity, rect2D);
            }
        }
        protected void DrawTmpSegment(Graphics graph, Pen pen, PointF point1, PointF point2, bool useTransform, bool temporary)
        {
            ...
            if (useTransform)
            {
                newLine.Draw(graph, pen, this.matrixValueToScreen, rect2D);
            }
            else
            {
                newLine.Draw(graph, pen, GraphControl.matrixIdentity, rect2D);
            }
        }
        ...
        {
            // Draw straight Line
            PointF screenPoint = GetScreenPointFromValuePoint(mouseValuePoint);
