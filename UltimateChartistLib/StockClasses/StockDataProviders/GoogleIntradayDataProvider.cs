    ...
    public class GoogleIntradayDataProvider : StockDataProviderBase, IConfigDialog
    {
        ...
        public override bool LoadIntradayDurationArchiveData(string rootFolder, StockSerie serie, StockSerie.StockBarDuration duration)
        {
            ...
            if (File.Exists(durationFileName))
            {
                ...
                if (values == null)
                    ...
                    StockLog.Write("LoadIntradayDurationArchiveData Cache File Found, current size is: " + values.Count);
        ...
        public override bool DownloadIntradayData(string rootFolder, StockSerie stockSerie)
        {
            if (System.Net.NetworkInformation.NetworkInterface.GetIsNetworkAvailable())
            {
                ...
                if (File.Exists(fileName))
                {
                    if (File.GetLastWriteTime(fileName) > DateTime.Now.AddMinutes(-4))
                        return false;
                }
                using (WebClient wc = new WebClient())
                {
                    ...
                    if (stockSerie.ShortName.Contains(':'))
                    {
                        ...
                        url = "https://finance.google.com/finance/getprices?q={code}&x={exchange}&i=300&p=60d&f=d,c,v,o,h,l&df=cpct&auto=1";
                    ...
                    else
                    {
                        ...
                        url = "https://finance.google.com/finance/getprices?q={code}&i=300&p=60d&f=d,c,v,o,h,l&df=cpct&auto=1";
