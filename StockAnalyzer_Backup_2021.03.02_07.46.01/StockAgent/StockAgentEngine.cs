...
namespace StockAnalyzer.StockAgent
{
    ...
    public class StockAgentEngine
    {
        ...
        {
            series = series.Where(s => this.Agent.Initialize(s, duration)).ToList();

            var refSerie = StockDictionary.Instance["CAC40"];
            refSerie.Initialise();
            foreach (var date in refSerie.Keys.Skip(minIndex))
            {
                var serieToSell = new List<Tuple<int, StockSerie>>();
                var serieToBuy = new List<Tuple<int, StockSerie>>();

                var nbPositions = this.Agent.TradeSummary.Trades.Count(t => !t.IsClosed);
                foreach (var serie in series)
                {
                    int index = serie.IndexOf(date);
                    if (index >= minIndex) // Date exists
                    {
                        this.Agent.Initialize(serie, duration);
                        var action = this.Agent.Decide(index);
                        switch (action)
                        {
                            case TradeAction.Nothing:
                                break;
                            case TradeAction.Buy:
                                serieToBuy.Add(new Tuple<int, StockSerie>(index, serie));
                                break;
                            case TradeAction.Sell:
                                serieToSell.Add(new Tuple<int, StockSerie>(index, serie));
                                break;
                            case TradeAction.PartSell:
                                break;
                            default:
                ...
                foreach (var serie in serieToSell)
                {
                    this.Agent.Initialize(serie.Item2, duration);
                    this.Agent.CloseTrade(serie.Item1);
                }

                nbPositions = this.Agent.TradeSummary.Trades.Count(t => !t.IsClosed);
                foreach (var serie in serieToBuy)
                {
                    if (nbPositions < maxPositions)
                    {
                        this.Agent.Initialize(serie.Item2, duration);
                        this.Agent.OpenTrade(serie.Item2, serie.Item1);
                        nbPositions++;
                    }
                    else
                    {
