...
{
    public partial class YahooIntradayDataProviderConfigDlg : Form
    {
        YahooIntradayDataProvider dataProvider = null;
        StockDictionary stockDictionary = null;

        bool needRestart = false;

        public YahooIntradayDataProviderConfigDlg(StockDictionary stockDico)
        {
            InitializeComponent();

            this.dataProvider = new YahooIntradayDataProvider();
            ...
            if (System.Net.NetworkInformation.NetworkInterface.GetIsNetworkAvailable())
            {
                try
                {
                    ...
                    using (WebClient wc = new WebClient())
                    {
                        ...
                        {
                            succeeded = true;
                            this.nameTextBox.Text = "INT_" + row[1].ToUpper();
                        }
                        else
                        {
                            succeeded = false;
