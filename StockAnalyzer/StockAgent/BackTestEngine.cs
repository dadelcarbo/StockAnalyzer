using StockAnalyzer.StockAgent.BackTests;
using StockAnalyzer.StockClasses;
using StockAnalyzer.StockData;
using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Diagnostics;
using System.Linq;

namespace StockAnalyzer.StockAgent
{
    public delegate void BackTestPerformedHandler(IBackTest backTest);
    public class BackTestEngine
    {
        public IBackTest Agent { get; set; }
        public Type AgentType { get; private set; }


        public StockTradeSummary BestTradeSummary { get; private set; }
        public IBackTest BestAgent { get; private set; }

        public event ProgressChangedEventHandler ProgressChanged;

        public event BackTestPerformedHandler BestAgentDetected;

        public event BackTestPerformedHandler AgentPerformed;

        public BackTestEngine(Type agentType)
        {
            this.AgentType = agentType;
        }

        public void GreedySelection(IEnumerable<StockInstrument> instruments, BarDuration duration, int minIndex, Func<StockTradeSummary, float> selector)
        {
            Stopwatch stopWatch = new Stopwatch();
            stopWatch.Start();

            // Calcutate Parameters Ranges
            IBackTest bestAgent = null;
            var agentParameters = StockAgentBase.GetParamRanges(this.AgentType);

            var stockInstruments = new List<StockInstrument>();
            if (ProgressChanged != null)
            {
                this.ProgressChanged(this, new ProgressChangedEventArgs(0, null));
            }

            int nbSteps = instruments.Count();
            int modulo = Math.Max(1, nbSteps / 100);
            int nb = 0;
            foreach (var instrument in instruments)
            {
                if (Worker != null && Worker.CancellationPending)
                    return;

                stockInstruments.Add(instrument);
                if (ProgressChanged != null && nb % modulo == 0)
                {
                    int percent = (nb * 100) / nbSteps;
                    this.ProgressChanged(this, new ProgressChangedEventArgs(percent, null));
                }
                nb++;
            }

            int dim = agentParameters.Count();
            var sizes = agentParameters.Select(p => p.Value.Count).ToArray();
            var indexes = agentParameters.Select(p => 0).ToArray();
            nbSteps = sizes.Aggregate(1, (i, j) => i * j);
            modulo = Math.Max(1, nbSteps / 100);
            for (int i = 0; i < nbSteps; i++)
            {
                if (Worker != null && Worker.CancellationPending)
                    return;
                if (ProgressChanged != null && i % modulo == 0)
                {
                    int percent = (i * 100) / nbSteps;
                    this.ProgressChanged(this, new ProgressChangedEventArgs(percent, null));
                }
                // Calculate indexes
                CalculateIndexes(dim, sizes, indexes, i);

                // Set parameter values
                this.Agent = (IBackTest)Activator.CreateInstance(this.AgentType);
                int p = 0;
                foreach (var param in agentParameters)
                {
                    param.Key.SetValue(this.Agent, param.Value[indexes[p]], null);
                    p++;
                }

                // Perform calculation
                this.Perform(instruments, minIndex, duration);

                // Select Best (after cleaning outliers)
                this.Agent.TradeSummary.CleanOutliers();
                var tradeSummary = this.Agent.TradeSummary;
                this.AgentPerformed?.Invoke(this.Agent);
                if (bestAgent == null || selector(tradeSummary) > selector(bestAgent.TradeSummary))
                {
                    bestAgent = this.Agent;

                    this.BestAgentDetected?.Invoke(bestAgent);
                }
            }
            stopWatch.Stop();

            // Get the elapsed time as a TimeSpan value.
            TimeSpan ts = stopWatch.Elapsed;
            string elapsedTime = String.Format("{0:00}:{1:00}:{2:00}.{3:00}", ts.Hours, ts.Minutes, ts.Seconds, ts.Milliseconds / 10);
            string msg = bestAgent.TradeSummary.ToLog(duration) + Environment.NewLine;
            msg += bestAgent.ToLog() + Environment.NewLine;
            msg += "NB Instruments: " + instruments.Count() + Environment.NewLine;
            msg += "Duration: " + elapsedTime;

            this.BestTradeSummary = bestAgent.TradeSummary;
            this.BestAgent = bestAgent;

            this.Report = msg;
        }

        public BackgroundWorker Worker { get; set; }
        public string Report { get; set; }

        public static void CalculateIndexes(int dim, int[] sizes, int[] indexes, int i)
        {
            int tmpIndex = i;

            for (int j = 0; j < dim; j++)
            {
                indexes[j] = tmpIndex % sizes[j];
                tmpIndex /= sizes[j];
            }
        }

        public void Perform(IEnumerable<StockInstrument> instruments, int minIndex, BarDuration duration)
        {
            try
            {
                foreach (var instrument in instruments)
                {
                    if (!this.Agent.Initialize(instrument, duration, minIndex))
                    {
                        continue;
                    }

                    var size = this.Agent.DataSerie.LastIndex;
                    for (int i = minIndex; i < size; i++)
                    {
                        switch (this.Agent.Decide(i))
                        {
                            case TradeAction.Nothing:
                                break;
                            case TradeAction.Buy:
                                this.Agent.OpenTrade(i + 1);
                                break;
                            case TradeAction.Sell:
                                this.Agent.CloseTrade(i + 1);
                                break;
                            default:
                                throw new ArgumentOutOfRangeException();
                        }
                    }

                    this.Agent.EvaluateOpenedPositions();
                }
            }
            catch (Exception exception)
            {
                StockAnalyzerException.MessageBox(exception);
            }
        }
    }
}
