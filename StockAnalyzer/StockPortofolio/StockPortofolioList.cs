...
namespace StockAnalyzer.Portofolio
{
    public class StockPortofolioList : System.Collections.Generic.List<StockPortofolio>, IXmlSerializable
    {
        ...
        public StockPortofolioList GetSimulationPortofolios()
        {
            if (this.Count(p => p.IsSimulation) == 0)
            {
                this.Add(new StockPortofolio(StockPortofolio.SIMULATION, 100000, true));
            }

            var stockPortofolioList = new StockPortofolioList();
            stockPortofolioList.AddRange(this.Where(p => p.IsSimulation));

            return stockPortofolioList;
        }
