...
namespace StockAnalyzerApp.CustomControl
{
   ...
   public class ListViewEx : ListView
   {
      ...
      public int GetSubItemIndexAt(int x, int y)
      {
         int index = -1;
         ListViewItem viewItem = GetItemAt(x, y);
         if (viewItem != null)
         {
            int i = 0;
            foreach (ListViewItem.ListViewSubItem viewSubtem in viewItem.SubItems)
            {
               if (i > 0 && viewSubtem.Bounds.Contains(x, y))
               {
                  index = i;
                  break;
               }
