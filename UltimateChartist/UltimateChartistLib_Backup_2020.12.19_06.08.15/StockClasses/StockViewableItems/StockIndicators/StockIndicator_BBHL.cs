...
{
    public class StockIndicator_BBHL : StockIndicatorBase
    {
        public StockIndicator_BBHL()
        {
        }
        ...
        public override void ApplyTo(StockSerie stockSerie)
        {
            ...
            FloatSerie emaSerie = stockSerie.GetIndicator(this.parameters[3] + "(" + (int)this.parameters[0] + ")").Series[0];

            FloatSerie EMAWidth = stockSerie.GetIndicator("EMAH(" + (int)this.parameters[0] + ")").Series[0] - stockSerie.GetIndicator("EMAL(" + (int)this.parameters[0] + ")").Series[0];

            upperBB = emaSerie + (float)this.parameters[1] * EMAWidth;
            lowerBB = emaSerie + (float)this.parameters[2] * EMAWidth;
